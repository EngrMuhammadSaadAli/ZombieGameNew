...
//						}
	...
	{
		int newindex = Random.Range (0, scores.Length);
		scores [currentindex].SetActive (false);
		currentindex = newindex;
		scores [currentindex].SetActive (true);
		coinsadd ();
	}

	public void coinsadd()
	{
		int newscore = Random.Range (100,250);
		currentscore = newscore;
		score = score + currentscore;
		soretxt.text = score.ToString ();
		PlayerPrefs.SetInt ("myscore", score);
		currentscore = 0;
		enemy.GetComponent<CharacterDamage> ().enabled = false;
	}
